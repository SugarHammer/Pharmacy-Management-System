	...
	function initTableData(){
		...
		if(listSubType == "调拨" || listSubType == "组装单" || listSubType == "拆卸单"){
			...
			else if(listTitle == "销售订单列表" || listTitle == "销售退货列表" || listTitle == "物流信息" || listTitle == "其它出库列表"){
				organNameTitle = "客户名称";
			}
		...
		$('#tableData').datagrid({
			...
			columns:[[
				...
				{ title: '药品信息',field: 'materialsList',width:200,formatter:function(value){
						if(value) {
	...
	function findStockNumById(depotId, meId, monthTime, body, input, ratio, type){
		...
		$.ajax({
			...
			success: function (rec) {
				if(rec && rec.code === 200) {
					var info = rec.data;
					var commodityUnit = info.commodityUnit; //药品单位
	...
	function initTableData_material(type,TotalPrice){
		...
		var isShowMaterialTypeColumn = true; //是否显示药品类型相关的列,true为隐藏,false为显示
		...
		$('#materialData').datagrid({
			...
			columns:[[
			  	{ field: 'Id',width:35,align:"center",checkbox:true},
				{ title: '药品类型',field: 'MType',editor:'validatebox',hidden:isShowMaterialTypeColumn,width:80},
				{ title: depotHeadName, field: 'DepotId', editor: 'validatebox', width: 90,
	          	...
	          	{ title: '条码_名称(规格)(备注)(扩展信息)(单位)',field: 'MaterialExtendId',width:270,
				  	formatter:function(value,row,index){
				  	...
				  	editor:{
						...
						options:{
							...
							onBeforeLoad: function(param){
								...
								param.mpList = mPropertyList; //药品属性
							},
							onLoadSuccess: function (rec) {
								if(rec && rec.total==1) {
									$(".datagrid-body [field='mBarCode']").click(); //在只有单个药品的时候自动选中
			...
			toolbar:[
                ...
                },
                {
                    ...
                    iconCls:'icon-add',
                    handler:function() {
                        js.addTabPage(null, "药品信息", "/pages/materials/material.html");
                        // appendMaterial(); //新增药品
                    }
                }
			],
                ...
                $.ajax({
                    ...
                    success: function (res) {
						if(res && res.code === 200) {
							...
							var purchaseDecimal = info.purchaseDecimal-0; //出厂价
                            ...
                            else if(listTitle == "销售订单列表" || listTitle == "物流信息" || listTitle == "销售退货列表" || listTitle == "其它出库列表" || listTitle == "调拨出库列表") {
	...
	function initTableData_material_show(TotalPrice){
		...
		$('#materialDataShow').datagrid({
			...
			onClickRow: onClickRow,
			columns:[[
				{ title: '药品类型',field: 'MType',width:80, hidden:isShowMaterialTypeColumn},
				{ title: depotHeadName,field: 'DepotName',editor:'validatebox',width:90},
				{ title: '条码_名称(规格)(备注)(扩展信息)(单位)',field: 'MaterialName',width:270},
	...
	function addDepotHead(){
		...
		if(pageType == "skip" && res) { //从订单跳转过来
			...
			depotHeadID = res.id;
			initTableData_material("edit",res.totalPrice); //药品列表
		} else {
			initTableData_material("add"); //药品列表
		}
	    reject(); //撤销下、刷新药品列表
	...
	function editDepotHead(index){
	    ...
	    initTableData_material("edit",TotalPrice); //药品列表
	    reject(); //撤销下、刷新药品列表
	...
	function showDepotHead(index){
        ...
        depotHeadID = res.id;
	    initTableData_material_show(TotalPrice); //药品列表-查看状态
	...
	function autoReckon() {
	    ...
	    setTimeout(function(){
			...
			body.find("[field='MaterialExtendId']").find(".textbox-text").focus().select(); //默认选择药品框
	    	//点击药品下拉框，自动加载数量、单价、金额
