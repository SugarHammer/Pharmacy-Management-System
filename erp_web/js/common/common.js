    ...
    function checkPower() {
        ...
        if(loginName == "admin") {
            $.messager.alert('提示','管理员不能增加表单数据，请使用其他账号进行操作！','warning');
            res = true;
        } else {
            res = false;
		}
	...
	function checkPowerForUser() {
		...
		var loginName = sessionStorage.getItem("loginName");
		if(loginName == "admin") {
			$.messager.alert('提示：本系统为多员工SAAS系统','请先到<a target="_parent" href="/register.html">注册页面</a>' +
				'进行员工注册，<br/>然后使用其他账号登录系统，<br/>最后通过员工去创建用户！','warning');
			res = true;
		} else {
			res = false;
		}
